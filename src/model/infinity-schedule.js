...
export default class OculusSchedule {
    ...
    update() {
        ...
        if (events.next.length && events.next[0].start_date - 60 * 10 <= this.state.now) {
    ...
    setupTimer() {
        ...
        if (this.state.events.next && this.state.scheduled_banners.next) { // Wait for next event or banner to start.
            ...
            return this.setTimeout(
                (this.state.events.next.start_date - 60 * 10 - this.state.now) * 1000
            )

<template>
    <div class="container-fluid px-7 pb-7 event" :class="{ next }">
        ...
        <div class="row">
            ...
            <div class="col-6 text-uppercase text-right" style="padding-top: 2.81em;" v-if="!next && current">
                <span class="highlight">Current Time: {{ schedule.getDate(schedule.state.now).format('HH:mm') }}</span>
            </div>
        </div>
        <div class="row">
    ...
    export default {
        ...
        computed: {
            eventTitleStyle: function() {
                return  {
                    'font-size': this.next ?
                    (this.event.title.length < 50 ? '3.78em' : '2.3rem') : (this.event.title.length < 30 ? '5em' : '3rem'),
                }
            },
            speakerName() {
